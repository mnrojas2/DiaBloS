...
def key_data(event, text, pointer):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            #Cambiar el número de puertos de un bloque (parte 2)       
                ...
                if input_pointer == 0:
                    text_edi, img_edi, input_pointer, rectc = key_data(event, text_edi, input_pointer)
                elif input_pointer == 1:
                    text_edo, img_edo, input_pointer, rectc = key_data(event, text_edo, input_pointer)
                    if input_pointer == 2:
                        for b_elem in blocks_list:
